...
function incompleteExplanation(report: AnalysisReport): string {
  if (report.analysisCompleteness === 'stoppedByLimit') {
    return 'Проверка остановлена защитным лимитом. Техническая остановка сама по себе не повышает Risk Score, но результат нельзя считать полным.';
  }
  if (report.analysisCompleteness === 'failed') {
    return 'Этот объект не содержит нового полного verdict и открыт только в безопасном режиме совместимости или ошибки.';
  }
  return 'Часть анализаторов или данных была недоступна. Учитывайте ограничения отчёта перед принятием решения.';
}
