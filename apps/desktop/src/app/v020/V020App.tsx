...
export function V020App() {
  ...
  return <div className={`app theme-${resolvedTheme}`}>
    <aside className={`sidebar ${preferences.sidebarCollapsed ? 'collapsed' : ''}`} aria-label="Основная навигация">
      ...
      <div className="sidebar-bottom"><button className="nav-item utility-item" onClick={() => navigate('about')} title="О программе"><CircleHelp /><span>О программе</span></button><small>Версия 0.3.1</small></div>
    </aside>
    <main className="main">
...
function HomePage({ navigate, chooseFile }: { navigate: (page: Page) => void; chooseFile: () => Promise<void> }) {
  ...
  return <>
    ...
    <section className="info-banner"><ShieldCheck /><div><strong>Объяснимый риск вместо надписи «безопасно»</strong><span>Каждый уровень риска формируется из конкретных правил, доказательств и рекомендаций. Одиночные системные API не считаются угрозой без опасной комбинации.</span></div></section>
  </>;
}
...
function AboutPage() {
  return <><PageHeader title="О программе" text="FileScope v0.3.1 — исправление ложных PE-вердиктов и Windows-иконок." /><section className="card about-v020"><span className="about-logo"><ShieldCheck /></span><div><h2>FileScope 0.3.1</h2><p>Desktop-приложение для предварительной локальной проверки файлов, ссылок и ZIP-архивов до запуска.</p></div><dl className="metadata-grid"><div><dt>Режим</dt><dd>Локальный анализ</dd></div><div><dt>Файлы</dt><dd>SHA-256, magic, PE</dd></div><div><dt>URL</dt><dd>Пассивный + ручной активный</dd></div><div><dt>Архивы</dt><dd>ZIP без извлечения</dd></div></dl><div className="status-banner warning"><Info /><span>FileScope учитывает комбинации признаков: одиночный VirtualAlloc или ShellExecute не считается доказательством вредоносности.</span></div></section></>;
}
